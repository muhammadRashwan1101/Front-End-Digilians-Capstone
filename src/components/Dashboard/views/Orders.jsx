...
export default function Orders() {
    return <>
        ...
        <h3 className="text-xl font-medium">Orders History</h3>
            ...
            {orders.map(order => (
                <div key={order.id} className="grid grid-cols-4 gap-5 border-b-2 py-10 border-gray-300 w-full">
                    <span>{order.id}</span>
                    <span>{order.placed_at}</span>
                    <span className={order.status === "delivered" ? "text-green-600" : order.status === "processing" ? "text-yellow-600" : "text-gray-600"}>{order.status}</span>
